...
function move(v, e)
  local curr = v.Tag
  local currView = v
  local ry = e.getRawY() -- 获取触摸绝对 Y 位置
  local rx = e.getRawX() -- 获取触摸绝对 X 位置
  if e.getAction() == MotionEvent.ACTION_DOWN then
    local lp = v.getLayoutParams()
    local vy = v.getY() -- 获取视图的 Y 位置
    local vx = v.getX() -- 获取视图的 X 位置
    local lastY = ry -- 记录按下的 Y 位置
    local lastX = rx -- 记录按下的 X 位置
