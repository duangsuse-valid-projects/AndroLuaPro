...
local LuaDrawable = luajava.bindClass "com.androluna.LuaDrawable"
...
local function setattribute(root, view, params, k, v, ids)
  ...
  elseif k == "onClick" then -- 设置 onClick 事件接口
    local listener
    if type(v) == "function" then
...
local function loadlayout(t, root, group)
  ...
  if style then
    ...
    view = t[1](context) -- 创建 view
  end

  local params = ViewGroup.LayoutParams(checkValue(t.layout_width) or -2, checkValue(t.layout_height) or -2) -- 设置 layout 属性
  if group then
  ...
  for k, v in pairs(t) do
    if tonumber(k) and (type(v) == "table" or type(v) == "string") then -- 创建子 view
      view.addView(loadlayout(v, root, t[1]))
    elseif k == "id" then -- 创建 view 的全局变量
      rawset(root, v, view)
      local id = ids.id
      ids.id = ids.id + 1
      view.setId(id)
