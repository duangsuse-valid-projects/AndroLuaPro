...
public class FloatWindow {
  ...
  private class TitleView extends TextView {
    ...
    public boolean onTouchEvent(MotionEvent e) {
      ...
      h = frame.top;
      ry = (int) e.getRawY(); // 获取触摸绝对 Y 位置
      rx = (int) e.getRawX(); // 获取触摸绝对 X 位置
      if (e.getAction() == MotionEvent.ACTION_DOWN) {
        vy = ry - (int) e.getY(); //获取视图的 Y 位置
        vx = rx - (int) e.getX(); //获取视图的 X 位置
        lastY = ry; // 记录按下的 Y 位置
        lastX = rx; // 记录按下的 X 位置
  ...
  private class ContentView extends LinearLayout {
    ...
    public boolean onTouchEvent(MotionEvent e) {
      ...
      if (e.getAction() == MotionEvent.ACTION_DOWN) {
        ...
        vw = getWidth(); // 获取视图的 Y 位置
        vh = getHeight(); //获取视图的 X 位置
        lastY = ry; // 记录按下的 Y 位置
        lastX = rx; // 记录按下的 X 位置
        vx = mLayoutParams.x;
        vy = mLayoutParams.y;
